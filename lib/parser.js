	...
	*init()
	...
	parseGame(scenes, cast)
	{
		...
		for (const sceneID in scenes)
		{
			...
			{
				throw new ParserError(sceneID, err.location, err.found, errorCode.syntaxError, err.message);
			}
			...
			const verifyNode = (node) =>
			{
				...
				switch (node.type)
				{
					...
					default: throw currentNodeError(errorCode.invalidNodeType, `Unhandled node type: ${node.type}`);
				}
