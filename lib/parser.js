	...
	parseGame(scenes, cast)
	{
		...
		for (const sceneID in scenes)
		{
			...
			const verifyBlockID = (node, blockID) =>
			{
				...
				{
					throw new ParserError(node, errorCode.blockNotFound, `Block ID "${blockID}" is invalid`);
				}
			};
			...
			const verifyNode = (node) =>
			{
				switch (node.type)
				{
					...
					case 'line':
					if (!cast[node.data.name])
					{
						throw new ParserError(node, errorCode.castNotFound, `No cast entry found for "${node.data.name}"`);
					}
					break;
					...
					case 'gotoScene':
					{
						...
						{
							throw new ParserError(node, errorCode.sceneNotFound, `Scene ID "${node.data.sceneID}" is invalid`);
						}
					}
					break;
					...
					default: throw new ParserError(node, errorCode.sceneNotFound, `Unhandled node type: ${node.type}`);
				}
	...
	recreateText(scene)
	{
		const recreateNode = function(node)
		{
			switch (node.type)
			{
				...
				case 'line': return `${node.data.name}: "${node.data.line}"`;
