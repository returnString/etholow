	...
	*handleGoto(node)
	{
		switch (node.type)
		{
			...
			case 'block':
			{
				return yield this.processBlock(node);
			}
			...
			case 'choiceList':
			{
				while (true)
				{
					...
					for (const choice of node.data.choices)
					{
						...
						const desc = choice.data.prefix === '>>' ? `"${choice.data.desc}"` : choice.data.desc;
						const answer = { available, desc, target: choice.data.target };
					...
					const selection = yield this.interface.showChoice(visualAnswers.map(a => a.desc), answerSource.length);
					...
					return yield this.handleGoto(answer.target);
				}
			}
